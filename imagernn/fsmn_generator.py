  ...
  """
    ...
    """
        ...
        for l in range(params.get('layers')):
            L = HLayer(d,drop_prob_decoder,predict_mode,l)
            finalH, cache,M = L.forward(H,n,model,cache)
    ...
    def backward(dY, cache):
        ...
        dWd = Hfinal.transpose().dot(dY)
        dbd = np.sum(dY, axis=0, keepdims=True)
        D = dY.dot(Wd.transpose())
        dict = {}
        for l in reversed(range(layers)):
            L = HLayer(d,drop_prob_decoder,None,l)
