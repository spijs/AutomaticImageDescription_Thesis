...
De best presterende systemen in deze masterproef zijn niet altijd in staat om perfect de afbeeldingen te beschrijven.
 De gegenereerde zinnen zijn wel vloeiend en grammaticaal correct.
  Figuur~\ref{fig:badresults}~en~\ref{fig:intermediateresult} in de appendix geven hiervan een aantal voorbeelden.
...
Dit is een reactie op de observatie uit dat tot slechts de helft van de zinnen echt uniek zijn. Bovendien is de woordenschat van de getrainde modellen steeds beperkt tot
...
De experimenten vergelijken gLSTM met als gids LDA met 120 onderwerpen met en zonder idf-normalisatie.
...
Tabel~\ref{table:idf-stats} toont dat idf-normalisatie het doel wel bereikt. De gemiddelde lengte verhoogt tot ongeveer het niveau van Gauss-normalisatie. 99\% van de gegenereerde zinnen komt niet voor in de trainingsverzameling. 93\% Van de zinnen is komt slechts \'e\'en keer voor in de trainingsverzameling en gegenereerde zinnen samen. Het is dus duidelijk dat de creativiteit van dit systeem een stuk hoger ligt dan zonder idf-normalisatie.
\begin{table}
	\centering
	\begin{tabular}{llllll}
		~                  & Bleu-1 & Bleu-2 & Bleu-3 & Bleu-4 & Meteor \\ \hline
...
De meeste afbeeldingen in de testverzameling genereren zowel correcte grammaticaal als inhoudelijk correcte zinnen.
...
Helaas gaat het systeem bij het genereren van veel zinnen duidelijk in de mist. Een vaak voorkomend probleem zijn woordherhalingen. Ook de woorden \texttt{African American} komen te vaak voor bij beschrijvingen waar mensen in voorkomen. Dit kan deels verklaard worden door dat de woorden \texttt{African} en \texttt{American} in hetzelfde onderwerp voorkomen als \texttt{white}, maar nu de voorkeur krijgen omdat ze weinig voorkomen in de trainingsverzameling. Voorbeelden van foutief gegenereerde zinnen zijn:\\
...
Deze vorm van normalisatie lijkt in vele gevallen creatievere, minder vage en soms menselijkere zinnen te genereren. Toch bevat het nog een groot aantal storende fouten. Verder onderzoek naar dit type normalisatie, misschien in combinatie met bijvoorbeeld Gauss-normalisatie, lijkt nuttig. 
...
\label{sec:besluit}
Uit dit hoofdstuk blijkt dat onze modellen zeker voldoen aan de vooropgestelde doelstelling om verbeteringen te brengen aan de startimplementatie. Het RNN-model verbetert door het toevoegen van LDA. Gauss-normalisatie zorgt voor nog betere en langere zinnen.
Ook het LSTM-model verbetert na toevoeging van zowel LDA als CCA. Het LSTM-netwerk leert een groter vocabularium in vergelijking met RNN. Ook hier zorgt Gauss-normalisatie voor nog verdere verbetering bij LDA en enkel voor de Meteor-score bij CCA.
Ondanks de verbeteringen vertonen beide types van systemen nog steeds enkele problemen bij het genereren van zinnen.
Vervolgens toont dit hoofdstuk aan dat de ideale beam-lengte afhangt van het al dan niet gebruiken van Gauss-normalisatie. Als compromis tussen de twee nemen we een grootte van 50.
Gauss-normalisatie speelt ook een rol bij de keuze van het aantal onderwerpen bij LDA. Wanneer Gauss-normalisatie aanwezig is lijkt een groter aantal onderwerpen voordeliger. Bij CCA is dit effect van de normalisatie niet aanwezig. De experimenten reiken 256 aan als ideale grootte voor de projectievector.

De experimenten tonen aan dat Gauss-normalisatie vrijwel steeds verbeteringen oplevert voor de hogere Bleu-scores en Meteor. Daarnaast cre\"eert het langere en iets creatievere zinnen. Min-Hinge-normalisatie daarentegen zorgt niet voor de verwachte verbeteringen.
