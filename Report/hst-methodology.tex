...
\todo[inline]{Dingen die ik nog mis: batches, solver, rmsprop}
...
De code van Karpathy voorziet ook de optie om gebruik te maken van ReLu encoders \todo{waar? kijk in code}, deze werken volgens formule $ReLu(x) = max(x,0)$. Er is ook de optie om te kiezen hoe groot de verborgen laag van het RNN moet zijn, alsook de optie om de afbeeldingsrepresentatie al dan niet tijdens elke stap toe te voegen.
...
Het leren van het gebruikte LDA model gebeurt op de Flickr30k dataset. Een voorbereidingsproces bewerkt de data zodat deze klaar is voor LDA. Dit proces combineert de vijf captions tot \'e\'en zin, verwijdert stopwoorden stemt de woorden met behulp van een Porter stemmer. \todo{reference naar porter stemmer mss best boek NLP} Deze zinnen vormen de documenten voor het LDA algoritme. Het doel is om op basis van een document te kunnen bepalen welke topics het sterkst aanwezig zijn. Het idee hierachter is dat een topicverdeling ervoor kan zorgen dat het systeem woorden genereert die binnen het juiste topic passen.

\todo[inline]{lastig om te bepalen hoe goed het is. DUS: 80 90 100 120 topics, namen geven om te zien of het logisch is voorbeeld van naamgeving}
...
Om evaluatie te vergemakkelijken hebben alle topics een naam gekregen. Op basis van de tien meest waarschijnlijke woorden, defini\"eren we voor elk topic een overkoepelend concept. \todo{voorbeelden} Een voorbeeld van een trainingsafbeelding met de meest waarschijnlijke topics is te zien in figuur \ref{fig:ldatopics}. Voor extra voorbeelden verwijzen we u door naar appendix \ref{app:LDA}.
...
\begin{figure}[h]
    ...
    \begin{tabular}{cl}
        ...
        \end{tabular}
    \end{minipage}
    \caption{Foto met de vijf meest waarschijnlijke topics -- correcte voorspelling}
...
Deze gewichten vertonen een verband met de hoeveelheid informatie die het woord bevat. \todo{aantal voorbeelden van begin en einde van de lijst toevoegen} Om een normalisatie te verkrijgen, telt het algoritme de gewichten voor alle afzonderljike woorden op. Op deze manier is er een afstraffing voor zinnen die veel frequente woorden bevatten en dus zo minder veelzeggend zijn.
\todo[inline]{voorbeelden + uitbreiden}
