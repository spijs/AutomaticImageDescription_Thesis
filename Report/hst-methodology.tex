...
\todo[inline]{inleiding moet herschreven worden zodat effectief de structuur van heel het hoofdstuk er in zit - ongeveer gedaan}
Dit hoofdstuk beschrijft hoe de eerder beschreven theoretische componenten samen een automatische generator van beschrijvingen kunnen vormen. Een eerste sectie handelt over een bestaande implementatie, dat het startpunt vormt van ons systeem. De volgende twee secties handelen over mogelijkheden om extra semantische informatie toe te voegen: LDA, het gebruik van de Flickr30k Entities dataset en CCA. Daarnaast beschrijft dit hoofdstuk ook uitvoerig de de toevoeging van deze informatie aan de bestaande netwerken. Enerzijds behandelt dit de toevoeging aan het eenvoudige RNN, anderzijds bespreekt het onze implementatie van het eerder vernoemde gLSTM netwerk zoals voorgesteld door Jia\cite{Jia2014}. De voorlaatste sectie beschrijft een normalisatiemethode voor beam-search zoals ook in dezelfde deze paper is beschreven. Het einde van dit hoofdstuk bespreekt nog een ander type van neuraal netwerk, dat de moeite waard was om te bekijken.

\section{Startpunt implementatie} \todo{Betere titel}
\todo[inline]{Dingen die ik nog mis:
		batches, solver, rmsprop, checkpoints, Python}
Het startpunt van onze implementatie is de code aangereikt door A. Karpathy op zijn GitHub pagina\footnote{\url{https://github.com/karpathy/neuraltalk}}. Die bevat een implementatie van het recurrente neurale netwerk beschreven in \cite{Karpathy2015}, alsook een implementatie die gebaseerd is op het werk van Vinyals et al.\cite{Google}. Vertrekkende van deze code hebben wij een aantal extensies ge\"implementeerd. \todo{Laatste zin voelt beetje te informeel :p}
...
\todo[inline]{Heel deze redenering slaagt op ni veel? dataformaat werkt ni goe voor neurale netwerken, dus gebruiken we een NEURAAL NETWERK om da om te vormen?}
...
De representatie die wordt gebruik door Karpathy is het resultaat van VGGNet voor het toepassen van de softmaxlaag. Dit leidt tot een 4096-dimensionele vector, waarbij elke dimensie kan worden gezien als een bepaald concept dat al dan niet aanwezig is in de afbeelding. Deze dimensionaliteit is dezelfde voor alle afbeeldingen, onafhankelijk van de grootte van de input. Dit komt door een herschaling alvorens de afbeeldingsvector te berekenen.
...
Voor het voorstellen van woorden stelt onder andere \todo{de juiste citen} vast dat het gebruik van word embeddings geen toegevoegd waarde brengt. Om die reden gebruikt Karpathy een one-hot encoding waarbij \'e\'en specifieke code behoort tot het start- en stopwoord. Vooraleer het woord als input dient, vindt eerst nog een vermenigvuldiging plaats met een gewichtsmatrix en een sommatie met een biasvector. Deze gewichtsmatrix en vector trainen mee tijdens de terugpropagatie van het netwerk. Op deze manier is het mogelijk dat semantische verbanden tussen de woordrepresentaties net als bij word embeddings, toch aanwezig zijn.
...
Op basis van een willekeurig gekozen afbeelding uit de trainingsset berekent het netwerk de beste zin. Het verschil tussen deze voorspelling en de correcte zin propageert terug door het netwerk en leidt tot de juiste aanpassing van de gewichten
 ...
 Wanneer de trainingsset volledig is doorlopen vindt een evaluatie plaats. De perplexity berekend op de resultaten voor de validatieset geeft snel aan of het netwerk nog bijleert of niet. \todo[inline]{Bij experimenten beschrijven hoe we de beste kiezen}
...
De training van het netwerk verloopt net als bij de RNN implementatie. Het netwerk leert op basis van de Flickr30k training set en bij elke volledige doorloop van de trainingset berekenent de software de perplexity van de resultaten op de validatieset. Dit kan dienen als een criterium om de training stop te zetten. Verder in deze thesis beschrijven we een betere manier om te bepalen wanneer de training moet stoppen.\todo{laatste zin zou k anders doen}

\todo[inline]{de volgende zin heb k hier gezet, maar het hoort ni echt ergens bij :P }
Karpathy voorziet in zijn code ook de mogelijkheid om de hyperbolische tangens te berekenen van de geheugenwaarde van de cel alvorens de output te berekenen. \todo[inline]{Is dit ni gewoon een andere activatiefunctie?}
...
\begin{figure}[!tb]
  ...
  \end{tabular}
  \todo[inline]{Reference naar entities paper}
