...
\label{par:training}
Het trainen van een feedforward netwerk gebeurt meestal met een techniek die terugpropagatie heet. Voor een netwerk met \'e\'en verborgen laag geldt het volgende voor de verborgen knopen $\mathbf{\hat{h}}$ en voorspelde output $\mathbf{\hat{y}}$ bij input $\mathbf{x}$:
\begin{equation}
...
\end{equation}
Waarbij $\mathbf{W}$ en $\mathbf{W'}$ de gewichten van de verbindingen tussen respectievelijk input en verborgen laag, en verborgen laag en output voorstellen. Het probleem bij het leren van $\mathbf{W}$ en $\mathbf{W'}$ is dat de correcte vector $\mathbf{h}$ niet gekend is. De training van het netwerk kan dus enkel op basis van de input en output gebeuren.
...
\begin{equation}
df = \frac{d}{d\hat{h}'}f(\hat{h}')
\end{equation}
In het geval van een logistische transferfunctie geeft dit volgende vector:
\begin{equation}
