...
\label{par:training}
Het trainen van een feedforward netwerk gebeurt meestal met een techniek die terugpropagatie heet. Deze techniek vergelijkt de verwachte output bij een gekende input met de berekende output. Op basis van dit verschil gebeuren er aanpassingen aan de gewichten in omgekeerde richting: eerst de gewichten van de laatste laag, dan de voorlaatste, \ldots 
...
\paragraph{Softmaxlaag}
De softmaxlaag is een laag die vaak als laatste laag in een neuraal netwerk voorkomt. Wanneer het netwerk meerdere outputs heeft, geeft de softmaxlaag een kansverdeling over de verschillende outputs terug in plaats van de oorspronkelijke outputvector. Concreet betekent dit dat de outputwaarden worden genormaliseerd op zo een manier dat elke waarde positief is en bovendien de som van alle waarden gelijk is aan 1. Dit is nuttig wanneer een kansverdeling over een aantal discrete componenten moet worden geleerd. Ook wanneer een input moet worden geclassificeerd, maar tot meerdere klassen kan horen is dit van nut.
...
Een CNN is een 'Deep Learning' architectuur bestaande uit verschillende niveaus. De input en output van elk niveau is een verzameling van arrays die men een 'feature map' noemt. Op elke output is de feature map een representatie van een bepaald kenmerk van de afbeelding. Elk niveau bestaat standaard uit drie lagen: een filter bank laag, een non-linearity laag en een feature pooling laag. Na een aantal van deze niveaus is er nog een classificatielaag.\todo{types lagen verder uitdiepen?}
...
\todo[inline]{wat is mcmc/gibbs, wat zijn dirichlet priors?}
\todo[inline]{Misschien hier ook al stilstaan bij hoe wij het net gebruiken? Van document naar topicverdeling}
\begin{figure}[tb]
    \centering
